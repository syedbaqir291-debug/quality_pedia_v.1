...
function openPanel(t){
  ...
  if(t.standardRefs&&t.standardRefs.length){
    html += `<div class="dp-section"><h3>Standards & References</h3>`;
    t.standardRefs.forEach(r=>{ html += `<div class="ref-chip">📘 ${r}</div>`; });
    html += `</div>`;
  }
...
function downloadPDF(){
  ...
  if(t.standardRefs&&t.standardRefs.length) addSection('Standards & References',t.standardRefs.map(b=>`• ${b}`).join('\n'));
...
function downloadExcel(){
  ...
  for(let c=0;c<6;c++)sc(ws1,addr(c,r),'',S.empty);r++;
  fillRange(ws1,r,0,5,'📘  STANDARDS & REFERENCES',S.secHdr);m1.push(mg(r,0,r,5));r++;
  (t.standardRefs||[]).forEach((ref,i)=>{
    fillRange(ws1,r,0,5,ref,i%2?S.bodyAlt:S.bodyWrap);m1.push(mg(r,0,r,5));ws1['!rows'][r]={hpt:22};r++;
  });
  for(let c=0;c<6;c++)sc(ws1,addr(c,r),'',S.empty);r++;
...
function downloadWord(){
  ...
  const refsHtml = (t.standardRefs||[]).map(r=>`<li>${r}</li>`).join('');
...
<h3>Standards & References</h3><ul>${refsHtml}</ul>
